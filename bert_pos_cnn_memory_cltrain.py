...
from models.Bert import Bert, Bert_entitystart
...
def cl_memory_triplet_train(args):
    ...
    for cls in range(0, 40):
        for idx, sample_idx in enumerate(random.sample(range(0, 560), args.K_shot)):
            (memory['x1'][cls * args.K_shot + idx], memory['x2'][cls * args.K_shot + idx], memory['x3'][
                cls * args.K_shot + idx], memory['pos1'][cls * args.K_shot + idx],
             memory['pos2'][cls * args.K_shot + idx]), memory['y'][cls * args.K_shot + idx] = train_dataset.__getitem__(
                cls * 560 + sample_idx)
    # 计算原型
    model.eval()
    ...
    with torch.no_grad():
        for i, ((x1, x2, x3, pos1, pos2), y) in enumerate(prototype_loader):
            x1, x2, x3, pos1, pos2, y = x1.to(device), x2.to(device), x3.to(device), pos1.to(device), pos2.to(
                device), y.to(device)
            prototypes[i] = model([x1, x2, x3], pos1, pos2).mean(dim=0)
    fix_prototypes = copy.deepcopy(prototypes)
    # 计算第一个session的精度
    session_data = Bert_Session(args.data_path, word2id, args.max_length, 0, args.K_shot, 'cl_test')
    ...
    with torch.no_grad():
        ...
        for batch_idx, ((x1, x2, x3, pos1, pos2), y) in enumerate(session_dataloader):
            x1, x2, x3, pos1, pos2, y = x1.to(device), x2.to(device), x3.to(device), pos1.to(device), pos2.to(
                device), y.to(device)
            dist = euclidean_dist(model([x1, x2, x3], pos1, pos2), prototypes[:70])
    ...
    for times in range(args.test_times):
        ...
        for i in range(1, 9):
            ...
            session_dataloader = DataLoader(session_data, batch_size=args.batch_size, shuffle=False)
            if args.random_test:
                (x1, x2, x3, pos1, pos2), y = session_data.get_random_train_session()
            else:
                (x1, x2, x3, pos1, pos2), y = session_data.get_train_session()
            # 存储当前session数据
            ...
            memory['x3'][memory_idx:args.N_way * args.K_shot + memory_idx], memory['pos1'][
                                                                            memory_idx:args.N_way * args.K_shot + memory_idx], \
            memory['pos2'][memory_idx:args.N_way * args.K_shot + memory_idx], memory['y'][
                                                                              memory_idx:args.N_way * args.K_shot + memory_idx] = x1, x2, x3, pos1, pos2, y
            ###
            x1, x2, x3, pos1, pos2, y = x1.to(device), x2.to(device), x3.to(device), pos1.to(device), pos2.to(
                device), y.to(device)
            ...
            if args.replay:
                for k in range(args.replay_epoch):
                    old_x1, old_x2, old_x3, old_pos1, old_pos2, old_y = torch.zeros(
                    ...
                    for idx, sample_idx in enumerate(
                        ...
                        old_x1[idx], old_x2[idx], old_x3[idx], old_pos1[idx], old_pos2[idx], old_y[idx] = memory['x1'][
                                                                                                              sample_idx], \
                                                                                                          memory['x2'][
                                                                                                              sample_idx], \
                                                                                                          memory['x3'][
                    ...
                    for idx, sample_idx in enumerate(
                        ...
                        old_x1[idx + args.replay_batch - 30], old_x2[idx + args.replay_batch - 30], old_x3[
                            idx + args.replay_batch - 30], old_pos1[idx + args.replay_batch - 30], old_pos2[
                    ...
                    old_x1, old_x2, old_x3, old_pos1, old_pos2, old_y = old_x1.to(device), old_x2.to(device), old_x3.to(
                        device), old_pos1.to(device), old_pos2.to(device), old_y.to(
                        device)
                    cl_optimizer.zero_grad()
                    logits = model([old_x1, old_x2, old_x3], old_pos1, old_pos2)
            ...
            with torch.no_grad():
                ...
                if args.sdc == 'all':
                    old_memory_output = old_model(
                        ...
                        memory['pos1'].to(device), memory['pos2'].to(device))
                ...
                if args.recompute_proto == 'all':
                    for cls in range(0, 40 + (i - 1) * args.N_way):
                        old_x1, old_x2, old_x3, old_pos1, old_pos2 = memory['x1'][
                                                                     cls * args.K_shot:(cls + 1) * args.K_shot], memory[
                                                                                                                     'x2'][
                                                                                                                 cls * args.K_shot:(
                                                                                                                                               cls + 1) * args.K_shot], \
                                                                     memory['x3'][
                                                                     cls * args.K_shot:(cls + 1) * args.K_shot], memory[
                                                                                                                 ...
                                                                                                                 cls * args.K_shot:(
                                                                                                                                               cls + 1) * args.K_shot], \
                                                                     memory['pos2'][
                                                                     cls * args.K_shot:(cls + 1) * args.K_shot]
                        old_x1, old_x2, old_x3, old_pos1, old_pos2 = old_x1.to(device), old_x2.to(device), old_x3.to(
                            device), old_pos1.to(device), old_pos2.to(device)
                        prototypes[cls] = model([old_x1, old_x2, old_x3], old_pos1, old_pos2).mean(dim=0)
                ...
                for batch_idx, ((x1, x2, x3, pos1, pos2), y) in enumerate(session_dataloader):
                    x1, x2, x3, pos1, pos2, y = x1.to(device), x2.to(device), x3.to(device), pos1.to(device), pos2.to(
                        device), y.to(device)
                    dist = euclidean_dist(model([x1, x2, x3], pos1, pos2), prototypes[:i * args.N_way + 40])
                    _, pred = torch.max(-dist, dim=-1)
                    correct += (pred == y).sum().float().item()
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--model_save_path', help='model存储路径', type=str,
                           default='result/temp_starttag_bert.pth') # base_bertcnn_pos_triplet_adam2e-5  base_bert_ft6_cnn_pos_triplet
    ...
    argparser.add_argument('--replay_epoch', help='重放任务轮次', type=int, default=30)
    ...
    # 可选参数为all、new、false
    argparser.add_argument('--recompute_proto', help='是否重计算原型', type=str, default='all')
    # 可选参数为all、base、false
    argparser.add_argument('--sdc', help='是否补偿语义漂移', type=str, default='false')
    ...
    print(args)
    print('重放批次:', args.replay_batch, '重放次数:', args.replay_epoch, '微调次数：',
          args.session_epoch, ' 正则化：', args.regularization, '  正则化系数：',
