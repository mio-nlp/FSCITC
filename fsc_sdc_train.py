...
def cl_train(args):
    ...
    base_out_put_proto = torch.zeros(80, args.encoder_dim).float().to(device)
    ...
    base_out_put_proto[:40] = prototypes[:40].clone()
    ...
    for i in range(1, 9):
        ...
        with torch.no_grad():
            ...
            base_out_put_proto[i * 5 + 35:i * 5 + 40] = base_model(sentence, pos1, pos2).view(5, args.K_shot, args.encoder_dim).mean(dim=1)
            new_class_proto = out_put.mean(dim=1)
            prototypes[i * 5 + 35:i * 5 + 40] = new_class_proto
            compose_prototypes[:i * 5 + 40] = torch.cat((base_out_put_proto[:i * 5 + 40], prototypes[:i * 5 + 40]), dim=1)
            # 在所有遇见过的类上测试
